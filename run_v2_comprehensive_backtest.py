...
def backtest_single_symbol_v2(args):
    """
    使用v2.0策略回测单个标的
...
def run_v2_comprehensive_backtest():
    """
    运行v2.0策略全面回测
    """
    print("=" * 80)
    ...
    with Pool(processes=num_processes) as pool:
        for idx, backtest_result in enumerate(pool.imap_unordered(backtest_single_symbol_v2, all_symbols), 1):
...
def generate_summary_report(results_df: pd.DataFrame, output_dir: str):
    ...
    # 保存统计报告
    report_file = os.path.join(output_dir, 'v2_summary_report.txt')
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("=" * 80 + "\n")
        f.write("v2.0策略全面回测统计报告\n")
        f.write("=" * 80 + "\n\n")
...
if __name__ == '__main__':
    results_df = run_v2_comprehensive_backtest()
