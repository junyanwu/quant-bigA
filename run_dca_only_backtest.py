...
def backtest_single_symbol_dca_only(args):
    ...
    try:
        ...
        if result:
            # 保存详细结果到JSON文件
            output_dir = f'./results/dca_only_results/{symbol}'
...
def run_dca_only_comprehensive_backtest():
    """
    运行纯定投策略全面回测
    """
    print("=" * 80)
    ...
    # 获取所有股票
    stock_dir = os.path.join(fetcher.data_path, 'stocks')
    if os.path.exists(stock_dir):
        stock_files = [f for f in os.listdir(stock_dir) if f.endswith('.csv')]
        for file in stock_files:
            symbol = file.replace('.csv', '')
            all_symbols.append((symbol, 'stock', start_date, end_date))
        print(f"找到 {len(stock_files)} 个股票")
    
    print(f"总计 {len(all_symbols)} 个标的")
    print("=" * 80)
    ...
    with Pool(processes=num_processes) as pool:
        for idx, backtest_result in enumerate(pool.imap_unordered(backtest_single_symbol_dca_only, all_symbols), 1):
    ...
    # 保存结果
    output_dir = './results/dca_only_comprehensive_backtest'
    os.makedirs(output_dir, exist_ok=True)
    
    # 保存详细结果
    results_df = pd.DataFrame(results)
    results_file = os.path.join(output_dir, 'dca_only_comprehensive_backtest_results.csv')
...
def generate_summary_report(results_df: pd.DataFrame, output_dir: str):
    ...
    # Top 10
    print(f"\n年化收益率 Top 10:")
    top10 = results_df.nlargest(10, 'annual_return')
    for idx, row in top10.iterrows():
        print(f"  {row['symbol']}: {row['annual_return']*100:.2f}%")
    
    # Bottom 10
    print(f"\n年化收益率 Bottom 10:")
    bottom10 = results_df.nsmallest(10, 'annual_return')
    for idx, row in bottom10.iterrows():
        print(f"  {row['symbol']}: {row['annual_return']*100:.2f}%")
    
    # 保存统计报告
    report_file = os.path.join(output_dir, 'dca_only_summary_report.txt')
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("=" * 80 + "\n")
        f.write("纯定投策略全面回测统计报告\n")
        f.write("=" * 80 + "\n\n")
...
if __name__ == '__main__':
    results_df = run_dca_only_comprehensive_backtest()
