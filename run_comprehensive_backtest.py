...
def backtest_single_symbol(args):
    ...
    try:
        ...
        if result:
            # 保存详细结果到JSON文件
            output_dir = f'./results/v1_results/{symbol}'
...
def run_comprehensive_backtest():
    ...
    # 保存结果
    output_dir = './results/comprehensive_backtest'
    os.makedirs(output_dir, exist_ok=True)
    
    # 保存详细结果
    results_df = pd.DataFrame(results)
    results_file = os.path.join(output_dir, 'comprehensive_backtest_results.csv')
